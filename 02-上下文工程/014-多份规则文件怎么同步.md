...
这就是「唯一真源 + 指针」能成立的机制基础。注意它只解决**一致性**，不解决上下文体积——导入的内容照样在启动时全量进上下文，所以真源本身也要控制在 200 行以内（见 [#010 CLAUDE.md 怎么写才真的生效](010-CLAUDE-md怎么写才生效.md)）。
...
所以 monorepo 里的做法是：根目录放跨包共用的，各 package 目录下放自己的，两边都别写「全仓所有细节」。相关坑见 [#050 大仓库 / monorepo 怎么让它不迷路](../06-工程协作/050-大仓库monorepo里不迷路.md)。
...
- [#075 团队的 AI coding 规范怎么立？哪些该强制、哪些该自由？](../08-团队与组织/074-团队AI-coding规范怎么立.md) —— 规范内容本身怎么定
