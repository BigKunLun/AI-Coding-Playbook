...
| 已经在付 ChatGPT 订阅，不想再开一份 | Codex CLI（含在 ChatGPT 订阅里） | 先看官方用量页：额度按 5 小时窗口加周上限计，官方未公布各档具体数字[^4] |
...
**大范围重构、要求风格贴合现有仓库的活给 Claude Code，日常改动且已在付 ChatGPT 订阅就用 Codex。**
...
| **Copilot** | 三档并存：行内补全 / IDE 内 agent mode / GitHub 云端 coding agent（可自主规划、执行、开 PR，2025-09 正式发布[^5]） | 适合已深度用 GitHub 生态的团队。云端 agent 自主度不低，但主入口仍是 IDE 插件与 GitHub 网页，不在你本地终端。 |
...
| 你的情况 | 选哪个 | 为什么 |
|---|---|---|
| 大范围重构、跨模块改、要求改出来的代码像原作者写的 | Claude Code | 社区对比里仓库级重构和深度代码库分析是它被反复提到的强项 |
...
- [Claude Code Docs: Quickstart](https://code.claude.com/docs/en/quickstart) —— 第 1、2 步的安装命令、四种登录方式、Console 自动建 workspace（官方，2026-08-15 复核）
- [Claude Code Docs: CLI reference](https://code.claude.com/docs/en/cli-reference) —— `--allowed-tools` / `--permission-mode` / `claude doctor` 的现行写法，支撑折叠块里的非交互模式；逗号 / 空格分隔均可这一条另经本地 `claude --help` 输出「Comma or space-separated list」确认（官方，2026-08-15 复核）
- [Claude Code Docs: Setup](https://code.claude.com/docs/en/setup) —— Homebrew cask `claude-code`、WinGet 包 `Anthropic.ClaudeCode`、两者默认不自动更新需手动 upgrade（官方，2026-08-15 访问）
- [Claude Code Docs: Enterprise deployment overview](https://code.claude.com/docs/en/third-party-integrations) —— 登录方式表里 Bedrock / Google Cloud's Agent Platform（原 Vertex AI）/ Microsoft Foundry / Claude apps gateway 的现行官方名称（官方，2026-08-15 访问）
- [Claude Code Docs: Model configuration](https://code.claude.com/docs/en/model-config) —— 1M 上下文窗口的支持模型与可用范围（随模型与订阅档位而异）（官方，2026-08-15 访问）
...
- [Copilot coding agent is now generally available — GitHub Changelog](https://github.blog/changelog/2025-09-25-copilot-coding-agent-is-now-generally-available/) —— 云端 coding agent 2025-09-25 GA 的官方公告（官方，2025-09）
- [Using Codex with your ChatGPT plan — OpenAI Help Center](https://help.openai.com/en/articles/11369540-using-codex-with-your-chatgpt-plan) —— Codex 含在各订阅档位、用量按 5 小时窗口加周上限计、未公布各档具体数字（官方，2026-08-15 访问）
...
- [Codex vs Claude Code — Northflank](https://northflank.com/blog/claude-code-vs-openai-codex)（社区，2025-09）/ [Dupple](https://dupple.com/learn/codex-vs-claude-code)（社区，2026-05）—— 两个终端 agent 的定性差异来自这类社区对比：Northflank 称 Claude Code 长于复杂重构与深度代码库分析，Dupple 称 Codex 执行更快、终端类 benchmark 更强；⚠️ 各家结论随模型版本变动，不作为硬结论使用
...
[^4]: [Using Codex with your ChatGPT plan — OpenAI Help Center](https://help.openai.com/en/articles/11369540-using-codex-with-your-chatgpt-plan)（官方，2026-08-15 访问）：Codex 含在 ChatGPT Free / Go / Plus / Pro / Business / Enterprise 各档内，本地消息与云端任务共用 5 小时窗口限额、另有周限额；官方未公布各档具体额度数字，撞顶后 Plus / Pro 可加购 credits。
[^5]: [Copilot coding agent is now generally available — GitHub Changelog](https://github.blog/changelog/2025-09-25-copilot-coding-agent-is-now-generally-available/)（官方，2025-09-25）：Copilot 云端 coding agent 对所有付费订阅正式发布。

---

<sub>难度 基础 · 决策题 + 配置题 · 主线 Claude Code，横向 Codex / ChatGPT / Cursor / Copilot</sub>

<sub>**时效**：安装命令（含 Homebrew / WinGet 包名与不自动更新行为）、四种登录方式（含 Console 自动建 workspace、apps gateway 登录流程）、`claude --version` / `claude doctor` / `/context` 输出、`--allowed-tools` 两种拼法与两种分隔、`--permission-mode` 七个取值（含 `manual` 需 v2.1.200+）已于 2026-08-15 依据官方 Quickstart、Setup、CLI reference 与本地 `claude --help` 核实，`--version` 输出取自本地 v2.1.220；Copilot 云端 coding agent GA 日期、Codex 订阅用量规则均已对到官方公告 / 帮助页。**已知不确定**：第 3 节依赖的两条社区经历（Reddit 帖、Phil Booth 站点）本轮未能实访逐字复核；Codex 与 Claude Code 的强项分工（重构 vs 速度）来自社区对比文章，非官方表述，也没有可复现的实测。**易变**：CLI flag 与权限档取值随版本变化，报错时以 `claude --help` 为准；云厂商入口的产品名近年多次改名（Google Cloud's Agent Platform 原 Vertex AI、Microsoft Foundry 原 Azure 系命名），以 third-party-integrations 官方页现行写法为准；Codex 各档额度规则与加购方式随 OpenAI 调价变动；两家 benchmark 排名每次模型更新就翻一次，本篇刻意不写具体分数；五工具对比是定性描述，以各自官方文档为准。</sub>
