...
**怎么确认生效**：`/mcp` 里那个 server 显示 disabled，且重开会话后 `/context all` 的 MCP tools 数字下降。**注意必须重开会话再量** —— 会话中途连/断 MCP 会让 prompt 缓存失效，当场量到的数字既不准，还额外烧一次钱（见 [#060](../07-效率与成本/060-一天该烧多少额度.md)）。
...
|---|---|
| 不设（默认） | 全部延迟加载，按需搜索 |
| `true` | 强制全部延迟，连代理和 GCP 上也发 beta 头 |
...
所以网上大量「装 5 个 MCP 吃掉 55K token」的说法，描述的是关闭延迟加载时的情况。Anthropic 自己给的那组示例数字是：5 个 server / 58 个工具约 55K token（GitHub 35 个工具约 26K，Slack 11 个约 21K），再加 Jira 约 17K 就奔着 10 万去了，内部见过 134K 的；开启 Tool Search Tool 后预加载从约 72K 降到约 500 token[^9]。**这组数字今天的正确读法是「上界」和「关掉延迟加载会退回到哪」，不是你现在的实际占用。**
...
2. **连接开销与启动阻塞**：`alwaysLoad` 的 server 会卡住启动最多 5 秒；HTTP/SSE 断开会按指数退避重连最多 5 次[^5]。装十几个远程 server，启动慢是必然的。
...
结论不变：不管哪个工具，「有 CLI 优先用 CLI、一次性任务不装 MCP」这条判据都成立，只是别的工具没给你量化和验证的手段。（横向部分为 2026-08 检索所得，未逐项在各家官方文档核对。）
...
- [CC Docs: Connect Claude Code to tools via MCP](https://code.claude.com/docs/en/mcp) —— 本篇绝大部分事实来源：三个 scope、`/mcp` 开关、`alwaysLoad`、`ENABLE_TOOL_SEARCH` 取值表、输出限额、重连策略（官方，2026-08 核对）
...
[^6]: [CC Docs: MCP · Configure tool search](https://code.claude.com/docs/en/mcp)（官方，2026-08 核对）：tool search 默认开启；在 Google Cloud Agent Platform、`ANTHROPIC_BASE_URL` 指向非官方主机时默认关闭；`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS` 会强制关闭且无法被 `ENABLE_TOOL_SEARCH` 覆盖；需模型支持 `tool_reference`（Sonnet 4.5 / Haiku 4.5 / Opus 4.5 及以后）。
...
[^9]: [Anthropic Engineering: Advanced tool use](https://www.anthropic.com/engineering/advanced-tool-use)（官方博客，2025-11）：5 个 server / 58 个工具约 55K token，加 Jira（约 17K）后超过 10 万，内部见过 134K；Tool Search Tool 把预加载从约 72K 降到约 500 token，整体减少约 85%。

---

<sub>难度 中级 · 决策题|配置题 · 主线 Claude Code，横向 Cursor / Copilot</sub>

<sub>**时效**：`/context all`、`/mcp enable|disable`、`alwaysLoad`、`ENABLE_TOOL_SEARCH` 取值表、`MAX_MCP_OUTPUT_TOKENS`、三个 scope 的存放位置均按 2026-08 官方文档核对。**已知不确定**：文中「几百 / 几千 / 上万」的三档分界是经验分档，未见官方给出推荐阈值，按你自己的窗口大小调；横向对比一栏为检索所得，未逐项在 Cursor / Copilot 官方文档核对。**易变**：tool search 目前走 beta 头，默认值和支持的模型列表都在变；`alwaysLoad`（v2.1.121+）、`/mcp` 非交互模式（v2.1.205+）带版本门槛，配之前先 `claude --version` 对一眼。</sub>
