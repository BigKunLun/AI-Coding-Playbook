...
**本篇画的是数据和密钥层面的组织红线，以及事故防护。** 它不重复讲单次产出的代码正确性校验，那在 [#040 谁来验证 AI 代码](../05-quality-assurance/040-who-verifies-ai-code-and-how.md)。它也不重复团队规范怎么立（[#075](./075-team-ai-coding-standards.md)）和成本（[#060](../07-efficiency-and-cost/060-how-much-does-claude-code-cost-per-day.md)）。
...
1. **用 managed settings 全员强制，而不是写进 CLAUDE.md 建议。** CLAUDE.md 是上下文而非配置，可被忽略（见 [#010](../02-context-engineering/010-claude-md-how-to-make-it-work.md)）。托管策略层用来放组织安全基线。
...
- ❌ **把安全红线写进 CLAUDE.md 当"强制"。** CLAUDE.md 是可被忽略的上下文（[#010](../02-context-engineering/010-claude-md-how-to-make-it-work.md)）。要强制，就用 managed settings 加 hook。✅
...
**交叉引用：**
- [#040 AI 说"做完了"、测试也全绿，怎么知道代码是真的对？](../05-quality-assurance/040-who-verifies-ai-code-and-how.md) —— 本篇"AI 代码本身不安全"那层的验证体系归它，本篇只画红线，不讲校验手法。
- [#075 团队的 AI coding 规范怎么立？哪些该强制、哪些该自由？](./075-team-ai-coding-standards.md) —— 安全基线是规范里最该强制的一档，规范怎么写、怎么强制归它。
- [#071 AI coding 的效能到底怎么度量？DORA / SPACE / 代码量 / AI 生码率，哪个不是 vanity metric？](./071-measuring-ai-coding-productivity-dora.md) —— 安全事故数可作为度量的反向指标之一。
- [#010 CLAUDE.md 怎么写才真的生效？](../02-context-engineering/010-claude-md-how-to-make-it-work.md) —— 为什么红线要用 managed settings 而非 CLAUDE.md。
- [#060 额度怎么突然就没了？20x Max 也撑不过一上午——先算清自己一天该烧多少](../07-efficiency-and-cost/060-how-much-does-claude-code-cost-per-day.md) —— 自托管 Bedrock/Vertex 的成本取舍归成本篇。
