# 075. 团队的 AI coding 规范怎么立？哪些该强制、哪些该自由？
...
本篇的边界：只讲规范的强制/自由怎么划，以及怎么用 Claude Code 的三层设置落地。相关但不在本篇展开的内容，各有去处。安全红线细节见 [#072](./072-ai-coding-security-red-lines.md)。review 流程怎么改见 [#053](../06-engineering-collab/053-ai-prs-too-large-to-review.md) 和 [#041](../05-quality-assurance/041-how-to-review-ai-generated-code.md)。token 账单和预算见 [07 主题](../07-efficiency-and-cost/060-how-much-does-claude-code-cost-per-day.md)。本篇讲「规矩写什么、怎么强制」。
...
| 强制项 | 为什么必须硬 | 落地手段（非纸面） |
|--------|-------------|-------------------|
| **安全红线**：禁止把密钥、客户数据、受监管数据、专有源码贴进未批准工具；禁止用个人或消费账号写公司代码 | 数据外泄不可逆 | managed policy 的 deny 规则 + secrets 扫描 + DLP，细节见 [#072](./072-ai-coding-security-red-lines.md) |
...
| **人对产出负责**：提交或批准 AI 辅助工作的开发者，对结果负责；开发者不得合并他解释不了的 AI 代码（[Metacto](https://www.metacto.com/blogs/creating-effective-ai-usage-policies-for-development-teams)）✅ | 责任不能转嫁给 AI | PR 模板 + review 纪律，机制见 [#040](../05-quality-assurance/040-who-verifies-ai-code-and-how.md) |
...
| **低风险场景的模型选择** | 内部工具或一次性脚本用哪个模型，交给成本判断，见 [07 主题](../07-efficiency-and-cost/060-how-much-does-claude-code-cost-per-day.md) |
...
> 触发条件必须无歧义，否则工程师会默认走最低档（[GoGloby](https://gogloby.com/insights/ai-policy/)）✅。review 流程本身怎么改（PR 体量、reviewer 疲劳）归 [#053](../06-engineering-collab/053-ai-prs-too-large-to-review.md) 和 [#041](../05-quality-assurance/041-how-to-review-ai-generated-code.md)，本篇只定「哪档触发强制」。
...
还有一条底层规则：deny 优先于 allow，且跨层时下层的 allow 翻不了案。这是安全红线能真正强制的关键，deny 细节归 [#072](./072-ai-coding-security-red-lines.md)。
...
1. 团队默认（should）写进仓库根的 `.claude/settings.json` 和项目 `CLAUDE.md`。命名、技术栈、错误处理约定都放这里，让 agent 统一续写。CLAUDE.md 怎么写见 [#010](../02-context-engineering/010-claude-md-how-to-make-it-work.md)。
...
4. 在 CI 挂 linter 和 secrets 扫描。纸面上的 must 只有挂上机器门禁才算真强制，这呼应了 [#040](../05-quality-assurance/040-who-verifies-ai-code-and-how.md) 的「约束层当独立真值源」。
...
**交叉引用：**
- [#072 AI coding 的安全红线怎么画？密钥泄露、数据外泄、第三方上传的真实事故与防护](./072-ai-coding-security-red-lines.md) —— 本篇「安全红线强制项」的细节与真实事故在 072。
- [#053 AI 生成的 PR 太大太多，review 机制怎么扛住不崩？](../06-engineering-collab/053-ai-prs-too-large-to-review.md) / [#041 怎么 review AI 生成的代码？（人审 + 机审策略）](../05-quality-assurance/041-how-to-review-ai-generated-code.md) —— 本篇只定「哪档触发强制人审」，review 流程本身在这两篇。
- [#040 AI 说"做完了"、测试也全绿，怎么知道代码是真的对？](../05-quality-assurance/040-who-verifies-ai-code-and-how.md) —— 把 linter 和扫描当「约束层独立真值源」挂进强制门禁。
- [#010 CLAUDE.md 怎么写才真的生效？](../02-context-engineering/010-claude-md-how-to-make-it-work.md) —— 团队默认（should）落地在 CLAUDE.md：文件级写法在 010，本篇是组织级政策框架。
- [#052 AI 写代码怎么和 git 流程配合？分支策略、commit、PR 怎么不让 AI 搞乱？](../06-engineering-collab/052-git-workflow-with-ai.md) —— commit 署名和披露的操作层。
- token 账单和预算上限 —— 见 [07 主题](../07-efficiency-and-cost/060-how-much-does-claude-code-cost-per-day.md)。
