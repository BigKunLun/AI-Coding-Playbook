...
一句话切边界：本篇回答"个人怎么用 AI 才不退化 + 团队人才结构怎么变"。团队的 AI coding 规范怎么立见 [#075](./075-team-ai-coding-standards.md)；效能怎么度量见 [#071](./071-measuring-ai-coding-productivity-dora.md)；"谁来验证 AI 代码"的体系见 [#040](../05-quality-assurance/040-who-verifies-ai-code-and-how.md)。
...
6. **提效和成长分开度量。** 产出量走 [#071](./071-measuring-ai-coding-productivity-dora.md) 的那套；成长用上面第 3 条的延迟测验，每月记一次分。别用产出涨来推断人在成长。
7. **立"用法红线"，不是"用量红线"。** 要禁的不是用 AI，是整块委托（wholesale delegation）不求理解。把上面那张提示词表塞进团队的 onboarding 和 `CLAUDE.md`（见 [#010](../02-context-engineering/010-claude-md-how-to-make-it-work.md)）。
...
机制详解见 [#032](../04-execution-workflow/032-subagent-when-and-how.md)。
...
结合 [#040](../05-quality-assurance/040-who-verifies-ai-code-and-how.md) 的"人最终 accountable（担责）"原则：提 PR 时要能口头讲清"AI 为什么这么写、你验证了什么"。这一步既挡住了走过场的橡皮图章式审查，也强制把理解补回来。上面 subagent 输出的第二段三个问题，正好可以当这一关的题库。
...
- ❌ **资深判断力只留在 senior 脑子里、靠口头带。** senior 一忙 junior 就裸奔。要用 subagent、`CLAUDE.md`、rules 把标准固化下来（[#010](../02-context-engineering/010-claude-md-how-to-make-it-work.md) / [#032](../04-execution-workflow/032-subagent-when-and-how.md)）。
...
**交叉引用：**
- [#075 团队的 AI coding 规范怎么立？哪些该强制、哪些该自由？](./075-team-ai-coding-standards.md) —— 本篇讲人才结构怎么变，075 讲团队规矩写什么、哪些该强制。
- [#071 AI coding 的效能到底怎么度量？DORA / SPACE / 代码量 / AI 生码率，哪个不是 vanity metric？](./071-measuring-ai-coding-productivity-dora.md) —— 本篇"提效≠成长"的度量落地在这。
- [#040 AI 说"做完了"、测试也全绿，怎么知道代码是真的对？](../05-quality-assurance/040-who-verifies-ai-code-and-how.md) —— 本篇"读懂才 merge"成长闸的质量体系版。
- [#010 CLAUDE.md 怎么写才真的生效？](../02-context-engineering/010-claude-md-how-to-make-it-work.md) —— 把资深判断力固化进上下文的载体。
- [#032 开几个 agent 并行跑，结果互相踩、被限流、token 指数级炸开——并行到底该开几个？](../04-execution-workflow/032-subagent-when-and-how.md) —— 把"资深 reviewer"做成可复用 agent 的机制。
