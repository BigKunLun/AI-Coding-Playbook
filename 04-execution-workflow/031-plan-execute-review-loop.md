...
为什么必须"对抗式"？因为写代码的上下文天然会替自己刚写的代码辩护。让同一个 agent 在同一个上下文里"自查"，它会顺着刚才的思路找理由。全新上下文的 reviewer 只看 diff 和验收标准，看不到写代码时的理由，所以能在自己的评估标准上客观打分。这就是"写代码的和打分的不能是同一个上下文"的工程价值，和 [#030 TDD](./030-tdd-in-ai-coding.md) 里"测试先于实现存在"是同源逻辑。
...
> 别把这条和他另一个更出名的说法混起来。Willison 的 "lethal trifecta"（致命三件套）讲的是 prompt injection 安全，指的是一个 agent 同时具备三样能力时会出事：能读你的私有数据、会接触到不可信内容、还能对外发消息（[The lethal trifecta](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/)，社区权威，2025-06）。那是安全议题，归 [#072](../08-team-and-org/072-ai-coding-security-red-lines.md)，和本篇的 review 纪律不是一回事。
...
3. 失败时修代码不修测试，见 [#030 TDD](./030-tdd-in-ai-coding.md) 的红绿循环。
...
**交叉引用：**
- [#030 TDD 在 AI coding 里怎么做？](./030-tdd-in-ai-coding.md) —— TDD 的 RED 阶段就是把验收标准前置，和 plan-first 循环同源。REVIEW 阶段的"证据非断言"和 TDD 的"watch it fail"一脉相承。
- [#033 Skill / superpowers 怎么用、怎么写？](./033-skill-superpowers-usage.md) —— brainstorming/writing-plans/executing-plans 三段链把循环强制化。
- [#032 开几个 agent 并行跑，结果互相踩、被限流、token 指数级炸开——并行到底该开几个？](./032-subagent-when-and-how.md) —— 对抗式 review 的 subagent 用法。
- [#012 上下文窗口要爆了怎么办？](../02-context-engineering/012-context-window-exploding.md) —— 长循环用 subagent，既隔离上下文又做对抗 review。
...
- [The lethal trifecta for AI agents — Simon Willison](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/) —— 私有数据 + 不可信内容 + 对外通信＝prompt injection 高危组合。本篇只用来做术语澄清，安全议题归 [#072](../08-team-and-org/072-ai-coding-security-red-lines.md)（社区权威，2025-06）
