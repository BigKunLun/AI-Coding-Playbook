...
2. **会话内的小回退用 `/rewind`。** 它只退得回 Claude 用文件编辑工具做的改动，适合「这一轮改歪了，退到上一步重来」。
...
- ❌ **在脏工作区上放 AI 大改，还把 `/rewind` 当回滚的全部依靠。** checkpoint 看不见 Bash 层的改动，AI 删掉的未跟踪文件它退不回来。兜底是动手前工作区干净 + `git stash -u`。
...
<sub>**时效**：所有配置片段与命令行参数已于 2026-08-03 逐条对照官方文档核实 —— `permissions` 的 allow/ask/deny 语法与求值顺序、`hooks.PreToolUse` 字段结构、`attribution` 三键、`--worktree` / `--from-pr` / `--allowedTools`。**已知不确定**：`includeCoAuthoredBy`「自某版本起弃用」的具体版本号官方未标注，本篇不写版本号；GitHub issue 的 👍 数是抓取当时的快照，会继续变。**易变**：配置键与 CLI 参数随版本迭代很快，动手前以官方文档为准。</sub>
