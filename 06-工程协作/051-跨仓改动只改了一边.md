...
| AI 改后端时不守后端仓的架构和命名约定 | 改用 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1 claude --add-dir ../backend` | 记住 `additionalDirectories` 永远不加载对方 CLAUDE.md |
| `/context` 的 Memory files 里没有 `../backend/CLAUDE.md` | 同上，并确认后端是用 `--add-dir` 加的而不是写在 `additionalDirectories` 里 | 该环境变量对 `additionalDirectories` 无效 |
| 后端能启动、返回 200、测试全绿，但没人验过 spec | 拿 spec 跑一遍 `schemathesis`，看响应字段名 / 类型 / 必填是否全对得上 | CI 里加 `oasdiff` 破坏性改动闸门 |
| 只跑了一边的测试就宣告做完 | 在工作区根跑 `./verify.sh` | 把两边的 typecheck + test 收进同一个脚本 |
...
```json
// 固定协作：frontend/.claude/settings.json（团队共享）或 settings.local.json（个人）
{ "permissions": { "additionalDirectories": ["../backend"] } }
```

仓多到 4 个以上、或要长期维护，再上「元仓库」工作区（见下面的折叠块）。

**怎么确认生效**：会话里跑 `/context`，看 **Memory files** 那一栏 —— 它列出当前加载了哪些 CLAUDE.md。

- 只跑 `claude --add-dir ../backend`：这一栏里**不该**出现 `../backend/CLAUDE.md`。
- 带上环境变量：这一栏里**应该**多出 `../backend/CLAUDE.md`。
- 带了环境变量仍没出现：确认后端是用 `--add-dir` 加的，而不是写在 `additionalDirectories` 里 —— 该环境变量对后者无效[^1]。
...
2. **立规矩：改接口先改契约。** 所有对 API 的改动，无论多小，都从更新规格开始[^4]。把这条写进工作区顶层 CLAUDE.md。
3. **让代码从契约生成，别手抄。** 改完 schema 就跑 codegen 产出类型和 client。代码是重新生成而不是手工编辑的，spec 和代码天生对齐。

把纪律写进 CLAUDE.md，AI 每次动接口才会从契约起步：
...
只当参考文档、不进 CI 的契约没有约束力 —— 没被强制执行的 spec 只是一份愿望文档。三道闸按需上：

- **spec-diff**：每个 PR 跑 `oasdiff` 检测破坏性改动（改名字段、删端点、改类型），一旦 breaking 就挡下。它还提供 MCP server，可以让 Claude Code 在动手前就对比两版 spec。
...
根因是结构性的：决定这次改动安不安全的依赖关系，横跨两个仓，落在任何单个仓的边界之外，也就落在 AI 的上下文窗口之外。这不是 Claude Code 独有的问题 —— GitHub 官方也承认 background agent「在单个仓的上下文里表现最好」，多仓建议拆成针对单仓的子任务[^5]。所以别指望换模型解决，只能靠工作区结构解决。

### 2. 官方给的是「文件访问」，不是「上下文合并」

这是最大的坑。`additionalDirectories` 和 `--add-dir` 给的只是文件读写权限，默认不会把那个目录的记忆和技能加载进来[^1]：

| 加法 | 加载 CLAUDE.md / rules | 加载 skills |
|---|---|---|
| `additionalDirectories` 设置 | 从不 | 从不 |
| `--add-dir` 标志 / `/add-dir` 命令 | 仅当设 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1` | 会 |

很多人抱怨「AI 改后端不守后端规范」，根因就在这：你给了它文件，没给它那个仓的说明书。
...
| **GitHub Copilot** | background agent 在单仓上下文里表现最好，官方建议拆成针对单仓的子任务，最后测集成点 | 同样靠 OpenAPI 或共享类型当真值源，加 CI 契约门；没有契约专属特性 | 多仓需手动拆解协调 |

共性趋势：三家都在补「跨仓依赖」这块短板，社区收敛到两类解法。一类是工作区打包，把依赖的仓拉进一个工作区，仓数量不超过 3 个时最省事；另一类是元仓库或显式依赖图，规模大或跨语言时才值得。别一上来就上重方案。
...
- [Set up Claude Code in a monorepo or large codebase — Claude Code Docs](https://code.claude.com/docs/en/large-codebases) —— 支撑「`additionalDirectories` / `--add-dir` 加载差异表」与「一次会话改完 + 计划落盘」两条法则（官方，2026-07）
...
- [alajmo/mani](https://github.com/alajmo/mani) —— `repos.yaml` 里 `desc` / `url` / `path` / `tags` 字段名的出处（开源工具，2026-08 核实）
- [Background agents touching multiple repositories? — GitHub community](https://github.com/orgs/community/discussions/186469) —— 支撑「跨仓不是 Claude Code 独有问题」，Copilot 官方建议拆成单仓子任务（官方论坛，2026）
...
- [oasdiff](https://www.oasdiff.com/) / [Schemathesis](https://schemathesis.io/) —— CI 三道闸的工具出处：前者做 OpenAPI 破坏性改动检测并提供 MCP server，后者从 spec 生成基于属性的用例验证运行中的实现（工具官网，2026）

[^1]: [Claude Code Docs: large-codebases](https://code.claude.com/docs/en/large-codebases)（官方，2026-07）：`additionalDirectories` 永不加载对方的 CLAUDE.md、rules 和 skills；`--add-dir` 加载 skills，只有设了 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1` 才加载 CLAUDE.md 和 rules，且该环境变量对 `additionalDirectories` 无效；两者都是读写权限。
[^2]: 同上：把整个改动放进一个会话，决策才一致；动手前把计划存成文件，它能活过被压缩的对话历史。
[^3]: [API Schema Validation Drift Detection Guide — TotalShiftLeft](https://totalshiftleft.ai/blog/api-schema-validation-catching-drift)（社区，2026）：`user_id`→`userId` 改名后 spec 未更新、三周后移动端生产崩溃，原文标注为示意场景。
...
[^5]: [Background agents touching multiple repositories? — GitHub community](https://github.com/orgs/community/discussions/186469)（官方论坛，2026）：background agent 在单个仓的上下文里表现最好，多仓场景建议拆成针对单仓的子任务。

---

<sub>难度 中级 · 配置题 + 流程题 · 主线 Claude Code，横向 Cursor / Copilot</sub>

<sub>**时效**：多仓配置事实（`additionalDirectories` 与 `--add-dir` 的加载差异、`CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` 的作用范围、两者都是读写权限）已于 2026-08-03 对照官方 large-codebases 文档逐条核实；契约与 CI 两节的事实性内容与外链于 2026-07-18 复核；`.gitignore` 写 `backend/` 而非 `backend/*` 为 2026-08 本地实测。**已知不确定**：「有 spec 让 LLM 生成代码错误率最高降 50%」是 arXiv:2602.00180 对若干 controlled studies 的转述，未逐一回溯原始研究，只看方向。**易变**：`--add-dir` 相关参数语义、oasdiff / Schemathesis / Spectral 的能力边界随版本变化，动手前以官方为准。</sub>
