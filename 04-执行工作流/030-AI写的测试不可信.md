...
2. `/goal` 不改权限，默认权限模式下每个工具调用照样问你；要真无人值守得配合 auto mode。
...
### 只在 prompt 里喊「请用 TDD」，回归反而变多

这条推翻了很多人的做法（在 CLAUDE.md 写一句「遵循 TDD」就当配好了）。一项在 SWE-bench Verified 上跑开源小模型的研究发现：只给 TDD 流程指令、不告诉模型「该验哪些测试」，回归率从基线 6.08% 被推高到 9.94%，比不干预还差；而先分析改动会影响哪些测试、再把这批测试喂给模型，回归率降到 1.82%[^4]。

结论很直接：小模型吃的是上下文信息，不是流程说教。必须给出具体要跑的命令和具体要变绿的测试文件，并且让它真跑、真贴输出 —— 上面每一步都围绕这一点设计。
...
- ❌ **只在 prompt 里喊「请用 TDD」。** 对开源小模型反而增加回归。要给命令和测试文件路径。
...
- [TDAD: Test-Driven Agentic Development — arXiv](https://arxiv.org/abs/2603.17973) —— 支撑「只喊 TDD 反而增加回归」的三个数字（研究，2026-03，单源）
...
<sub>**时效**：`Stop` hook 的 JSON 结构与退出码语义、`/goal` 命令语法与 v2.1.139 版本门槛、Cursor `.mdc` 的三个 frontmatter 字段，已于 2026-08 对照官方文档核实；TDAD 的三个回归率数字与包幻觉率（5.2% / 21.7%）已于 2026-07-18 核对原文。**已知不确定**：TDAD 是单一研究来源，仅在开源小模型上验证，不代表 Opus/Sonnet 级模型的行为；包幻觉率经 Augment Code 转引，与 042 篇引的是同一篇论文，不是两项独立研究。**易变**：`/goal` 的可用版本、hook 字段名与默认 timeout、连续 8 次的上限值，随 Claude Code 小版本变化，用前回查官方 hooks 文档。</sub>
