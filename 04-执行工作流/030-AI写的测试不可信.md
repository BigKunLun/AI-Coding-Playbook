...
**动实现前先有一个跑红的检查**；红了只改实现，`git diff --stat -- tests/` 必须为空。
...
**怎么判断这一步做对了**：可执行的检查跑一遍，退出码非 0；截图对比这类不产生退出码的形态，等价信号是对比清单里至少列得出一条当前不满足的差异。如果检查现在就是绿的，或者你根本写不出这个检查，说明验收标准还没细到能交给 agent，先回去补规格（见 [031 篇](./031-plan-execute-review循环.md)）。
...
tdd-guard 值得单独说一句，因为它堵的正是上面第一种作弊模式，而且不靠模型自觉：三条命令装好 —— `/plugin marketplace add nizos/tdd-guard`、`/plugin install tdd-guard@tdd-guard`、`/tdd-guard:setup`，支持 vitest / jest / pytest / PHPUnit / Go / Rust / Ruby 等。**怎么确认生效**：不写任何测试，直接让 Claude 新建一个实现文件，写入应当被拦下并提示先补失败测试；它照常写进去了，说明 setup 没跑完或测试框架没被识别。

<details>
<summary>用 /goal 把验收设成会话级停止条件（含条件写法与三个坑）</summary>

`/goal` 后面直接跟条件，回车即刻开跑，每轮结束由一个独立评估器复查，没达标自动开下一轮。需要 Claude Code v2.1.139 以上，命令不存在说明版本太旧，升级即可。
...
# 取消：clear 是正式写法，stop / off / reset / none / cancel 都是它的别名
/goal clear
```
...
<details>
<summary>四类作弊模式速查表（症状与堵法）</summary>
...
- ❌ **只防「改测试」这一种作弊，或让写代码的上下文自己给自己打分。** 串味、水平切片、自我祝贺在 diff 里看不见，agent 自查又会顺着刚才的思路替自己辩护 —— 要靠上下文隔离和全新上下文的子 agent 做对抗式审查。
...
两个已知限制：社区多有反馈改完 `.mdc` 不即时生效、要重开窗口（官方文档未见确证，遇到规则不生效先试这一招）；而且它和 CLAUDE.md 一样是建议性的，没有 Stop hook 那种退出码闸门。
...
<details>
<summary>参考资料（15 条，含来源类型与日期）</summary>

- [Best practices for Claude Code — Claude Code Docs](https://code.claude.com/docs/en/best-practices) —— 支撑「测试是 agent 的停止条件」「要证据不要断言」「对抗式审查」与验证闭合各档位（官方，2026-06）
...
- [A Claude Code TDD Skill: Forcing Red-Green-Refactor Discipline — alexop.dev](https://alexop.dev/posts/custom-tdd-workflow-claude-code-vue/) —— 单 context 串味模式与上下文隔离方案（社区，2025-11）
- [My Skill Makes Claude Code GREAT at TDD — aihero.dev（Matt Pocock）](https://www.aihero.dev/skill-test-driven-development-claude-code) —— 水平切片 vs 垂直切片（社区，2026-02）
- [From Sequential to Parallel: Building a Claude Code Harness — Level Up Coding（Leonid Sokolovskiy）](https://levelup.gitconnected.com/from-sequential-to-parallel-building-a-claude-code-harness-0a65ce89738a) —— 自我祝贺模式与独立评审 agent（社区，2026-04）
...
[^2]: [TDD, AI agents and coding with Kent Beck](https://newsletter.pragmaticengineer.com/p/tdd-ai-agents-and-coding-with-kent)（社区权威，2025-06）：文章明写 Beck 难以阻止 agent 删测试让套件变绿，并称 TDD 是与 agent 协作的「超能力」；「agent 想先写代码再补测试」出自访谈转述，公开文本中未见 Beck 原句（未见官方确证）。
...
[^5]: [Automate actions with hooks — Claude Code Docs](https://code.claude.com/docs/en/hooks-guide#stop-hook-hits-the-block-cap)（官方，2026-08-15 核实）：Stop hook 连续 8 次无进展地阻止结束后，Claude Code 会覆盖 hook 强制结束回合；上限可用环境变量 `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` 调高。
...
[^7]: [Auto mode is now the default in Claude Code — Anthropic 官方博客](https://claude.com/blog/auto-mode-default-in-claude-code) 与 [CC Docs: Permission modes](https://code.claude.com/docs/en/permission-modes)（官方，2026-08-15 核实）：2026-08-14 起 auto 成为 Pro / Max / Team 新会话的默认权限档；自设的 `defaultMode` 除非接受一次性切换提示否则保持不变，组织托管的默认档不变；Bedrock / Vertex / Foundry / 网关通道上 auto 只是进入 `Shift+Tab` 循环，起始档仍是 `defaultMode`。

---

<sub>难度 中级 · 流程题 · 主线 Claude Code，横向 Cursor / Copilot</sub>

<sub>**时效**：`Stop` hook 的 JSON 结构、退出码语义与连续 8 次上限、`/goal` 命令语法（别名、4000 字符上限、评估器不跑命令）、auto 默认档生效日期、Cursor `.mdc` 的三个 frontmatter 字段，已于 2026-08-15 对照官方文档核实；`/goal` 的 v2.1.139 版本门槛出自官方 changelog 的社区转录，官方文档页本身未标注版本号。TDAD 的三个回归率数字已于 2026-08-15 对照 arXiv 摘要复核。tdd-guard 的安装命令与框架支持范围已于 2026-08-15 对照仓库 README 核实，本篇作者未逐条实跑。**已知不确定**：TDAD 是单一研究来源，仅在开源小模型上验证，不代表 Opus/Sonnet 级模型的行为，正文已就地标注该边界；四类作弊模式出自三位个人博主的观察，无对照实验数据；Kent Beck「先写代码再补测试」为访谈转述，公开文本未见原句；Cursor 规则改后需重开窗口为社区经验，官方文档未见确证。**易变**：`/goal` 的可用版本、hook 字段名与默认 timeout、连续 8 次的上限值，随 Claude Code 小版本变化，用前回查官方 hooks 文档。</sub>
