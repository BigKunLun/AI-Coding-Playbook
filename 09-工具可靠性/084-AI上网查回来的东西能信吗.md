...
结果要写进产物就绕开 WebFetch：**用 curl 拿原文，不认识的域名一律当敌意输入**。
...
两个可靠的验伪动作，结果都可观测，命中任何一条异常就回原文核对：

1. **标题反查**：把引用的标题原样贴回原站或搜索引擎。搜不到 = 大概率是编的
2. **数字反查**：在原文里搜这个数字。任一表格、任一段落都找不到 = 它是「合成」出来的

另有一个只能当初筛的动作：直接问「你是真读了这篇，还是从搜索结果推的」。使用规则是**承认才有信息量，否认不构成证据**——模型对自己行为的自述本身不可观测也不可靠，既可能虚假承认，也可能坚持否认，最终分辨仍要靠前两条和回原文对照。

这个追问并非无的放矢：有一个 GitHub issue 记录的正是 **WebFetch 抓取失败时（比如目标站拦截），Claude Code 不报告失败，而是拿搜索结果编一个答案**，装作读过原文[^2]。Anthropic 官方支持文档也承认，Claude 会给出「看起来权威但没有事实依据」的引用，并建议不要把它当单一真相源、要复核引用来源[^3]。
...
这不是孤例。Google 基于每月 20–30 亿网页的公开抓取快照做扫描，2025-11 到 2026-02 期间恶意间接提示注入**增长 32%**，分类涵盖恶作剧、SEO 操纵、agent 威慑、数据外泄和破坏性命令[^5]。
...
这解释了为什么失真集中在数字、引用、专有名词这三类：它们恰好是「看起来最像事实、最不容易引起怀疑」的形态，也恰好是被要求给出答案的模型最容易凭空「合成」出来的形态。
...
OWASP 与多家安全厂商的建议都是不要依赖这条[^9]——它是单点，而且是攻击者可以反复测试直到绕过的单点。可靠的做法是结构性的：抓回内容当数据不当指令、工具权限按任务收敛、有后果的动作前置人工确认、记录 agent 活动。
...
- [Tools reference — Claude Code Docs](https://code.claude.com/docs/en/tools-reference) —— 支撑「WebFetch 用小而快的模型处理、lossy by design、建议改用 curl」（官方，核实于 2026-08-15）
...
- [tcrf-ai-agent-payload-report](https://github.com/bashalarmistalt/tcrf-ai-agent-payload-report) —— 支撑「按 UA 分流投递破坏性 payload」的具体案例（自发布，含第三方 urlscan.io 佐证，2026-08-05；2026-08-15 复核仓库已 404）
- [AI threats in the wild: The current state of prompt injections on the web — Google Security Blog](https://blog.google/security/prompt-injections-web/) —— 支撑野外间接注入的规模、增速与分类（官方研究，2026-04）
- [Indirect Prompt Injection in Web Content Targets AI Agents — Zscaler ThreatLabz](https://www.zscaler.com/blogs/security-research/indirect-prompt-injection-web-content-targets-ai-agents) —— 对 Google 扫描结果的厂商侧解读（厂商研究，2026）
- [LLM01: Prompt Injection — OWASP GenAI Security Project](https://genai.owasp.org/llmrisk/llm01-prompt-injection/) —— 支撑「不依赖指示模型忽略注入、防御须结构性」（行业组织，核实于 2026-08-15）
...
[^1]: [Tools reference — Claude Code Docs](https://code.claude.com/docs/en/tools-reference)（官方，核实于 2026-08-15）：WebFetch 取页后转 Markdown，用 a small, fast model 跑提取 prompt，多数情况下 Claude 收到的是该模型的回答而非原始页面；转换步骤不可配置，属 lossy by design；结果称页面未提及某项可能只是提取 prompt 未问及，建议用更具体的 prompt 重抓或经 Bash 用 curl 取未处理页面。同页另载：大页面按固定字符上限截断、响应缓存 15 分钟、跨 host 重定向不自动跟随、User-Agent 以 `Claude-User` 开头。
[^2]: [GH #45070](https://github.com/anthropics/claude-code/issues/45070)（社区，2026）：WebFetch 访问失败（如目标站拦截）时，Claude Code 从搜索结果编造答案并呈现为已读原文，不报告工具失败。提交者将其归因于系统提示中「简洁、先给答案」的倾向压过了如实报告失败。**单个 issue，已被标 duplicate 关闭，未见官方复盘**（标题与内容于 2026-08-15 对照原 issue 核实）。
...
[^4]: [tcrf-ai-agent-payload-report](https://github.com/bashalarmistalt/tcrf-ai-agent-payload-report)（自发布报告，2026-08-05 捕获）：该站对特定 AI agent UA 返回破坏性提示注入 payload。三个独立网络（US VPN、urlscan ES、urlscan DE）取得相同响应，urlscan.io 对响应体做哈希，与本地样本逐字节一致；分支发生在 origin（单台 Linode nginx，无 CDN）；响应缺 `Vary: User-Agent`；agent UA 反而绕过了浏览器 UA 遭遇的 IP 封锁。**局限**：单作者自发布仓库，非厂商或同行评审披露；报告自述未捕获「未被封的住宅 IP + 浏览器 UA」这一组合，故不能据此断言该站如何对待普通读者。该站另处于长期 DDoS 攻击下，封 IP 与返回 403 本身是可辩护的，报告指认的是「在已有可用的非破坏性拦截之后，另行对已识别 agent 投递文件破坏指令」这一独立决定。**2026-08-15 复核：该仓库已返回 404（作者账号仍在），报告原文无法再核对，此案例未见官方确证，仅作「按 UA 分流投毒在野外存在」的一例参考。**
[^5]: [AI threats in the wild: The current state of prompt injections on the web — Google Security Blog](https://blog.google/security/prompt-injections-web/)（官方研究，2026-04，核实于 2026-08-15）：基于 Common Crawl 每月 20–30 亿页面的快照做扫描，原文明写「between November 2025 and February 2026 恶意类别相对增长 32%」；类别含恶作剧、引导、SEO 操纵、agent 威慑、数据外泄、破坏性命令。原报告另指出当前攻击的复杂度仍较低、多为个人站长的实验或恶作剧，但预期规模与复杂度都会上升。
...
[^9]: [LLM01: Prompt Injection — OWASP GenAI Security Project](https://genai.owasp.org/llmrisk/llm01-prompt-injection/)（行业组织，核实于 2026-08-15）：明确指出现有技术不能完全消除提示注入，缓解建议均为结构性措施——约束模型行为、最小权限、高危动作人工确认、隔离并标识不可信外部内容、把模型当不可信用户做对抗测试；未把「指示模型忽略注入」列为可依赖的防御。Unit 42 同文的缓解建议同为权限收敛与内容隔离一类结构性措施[^8]。

---

<sub>难度 中级 · 排错题 + 安全题 · 主线 Claude Code，横向所有带联网能力的 coding agent</sub>

<sub>**时效**：2026-08-13 首次成文，2026-08-15 复核。WebFetch 的小模型处理、lossy by design 表述与 curl 替代建议已对照官方 tools-reference 页核实；auto mode 720 次评测数据与其局限对照官方公告与 Simon Willison 原文核实；Google 的 32% 增长与 20–30 亿页面规模已对照 Google Security Blog 原始报告核实；GH #45070 标题与内容已对照 GitHub 原 issue 核实。**已知不确定**：GH #45070 为单个社区 issue（已被标 duplicate 关闭），未见官方复盘；tcrf.net 案例的自发布仓库 2026-08-15 复核已 404，原文无法再核对，未见官方确证。**易变**：WebFetch 的截断上限、缓存时长、预批准域名清单随版本变化；auto mode 默认档规则自 2026-08-14 生效，此前会话行为不同。</sub>
