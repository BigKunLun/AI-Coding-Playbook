...
**就算人确实在电脑前，也拦不住多少。** 一个大样本公开实验测过这个环节的实际拦截率：人工审批漏过的威胁接近三分之一，最容易被放行的是伪装成 `npm run analyze` 这类日常命令的那一批，而且同一场会话里越往后漏得越多 —— 疲劳是可测量的，不是感觉[^8]。指向的结论是：**人工确认不是安全边界，它是通知机制**，把它当边界等于把安全性押在「你这次刚好没走神」上。
