...
先定性再改接入：**429 补席位，401/403 才停中转改接入，别把限流当封禁治**。
...
- 报未知模型名 → 凭证是好的（认证不通过时轮不到报模型错误[^5]），只是模型 ID 写错或过期，换成官方模型列表里的当前 ID 即可，不影响定性结论。
...
增加额度的合法路径只有两条：每人独立席位，或转 API 计费（第 3 步）。**转 API 计费不是没有限额**——它有按分钟滚动的速率限制（请求数和 token 数）加每月消费上限，高并发时同样会撞 429；区别在于这些限额随用量档位自动提升、也能在控制台申请调高[^4]，不像订阅那样按固定窗口把全队卡死。代价是成本要自己盯（见 [#060 额度怎么突然就没了](../07-效率与成本/060-一天该烧多少额度.md)）。
...
三选一，按两个能直接回答的问题走：**有没有人力运维一个网关？没有的话，有没有现成的云账号？**（与本篇开头的流程图一致。）

**方案一：LLM 网关（有人力运维）。** 密钥留服务端，每人发一把网关凭证，人走了吊销一把即可。开发者机器上只要两个变量：
...
凭证变量选哪个取决于网关认哪个 header，不确定先用 `ANTHROPIC_AUTH_TOKEN`，401 了再换（对照规则见下方折叠块）。
...
**方案三：每人一份订阅席位（既没人力运维网关、也没有云账号）。** 最贵也最省事，要点只有一条：一个账号只服务一个人，不共享、不转售、不搭池子。

<details>
<summary>三层验证：怎么确认真的切干净了（三层缺一层就定位不到问题）</summary>

凭证变量对照：网关说 bearer token / Authorization 就用 `ANTHROPIC_AUTH_TOKEN`（走 `Authorization: Bearer`）；说 API key / x-api-key 就用 `ANTHROPIC_API_KEY`（走 `x-api-key`）；凭证会轮换或来自 vault 就用 `apiKeyHelper`（两个 header 都发）。
...
限流是容量问题，加钱或加席位能解；封禁是政策问题，加钱解不了；第三方 harness 报错是兼容性问题，换接入方式能解。

<details>
<summary>三种中断对照表（症状 → 根因 → 性质）</summary>
...
<details>
<summary>档位不匹配 / 能力条款变更 / 单点依赖：三种都不是封号，但一样能让工作流停摆</summary>

- **档位不匹配**：社区反映团队版没有跟个人 Max 20x 对齐的高档位，结果重度用户只能留在个人账号上，组织拿不到 SSO 和统一管理[^3]。
- **能力条款会变**：订阅能不能配合第三方工具用，2026 上半年改过两次[^6]。工作流建立在某条政策上，那条政策就是你的依赖。
...
- [Claude Code — Legal and compliance](https://code.claude.com/docs/en/legal-and-compliance) —— 支撑「OAuth 适用范围、禁止第三方代路由订阅凭证、可不经通知执行」（官方，2026-08-15 核对）
- [Claude Code — Connect Claude Code to an LLM gateway](https://code.claude.com/docs/en/llm-gateway-connect) —— 支撑网关环境变量、凭证变量对照、验证请求与错误对照表、WAF 拦请求体、项目级 env 块在首次运行向导后才生效、Remote Control / 语音在网关凭证下不可用、Slack 与 web 永远走 Anthropic API（官方，2026-08-15 核对）
- [Claude Code — Other LLM gateways](https://code.claude.com/docs/en/llm-gateway) —— 支撑「只设 base URL 不替换订阅凭证」（官方，2026-08 核对）
- [Claude Code on Amazon Bedrock](https://code.claude.com/docs/en/amazon-bedrock) —— 支撑 Bedrock 变量、「多人部署必须钉模型版本」的警告、以及 WebSearch / `/logout` 在 Bedrock 上不可用（官方，2026-08-15 核对）
- [Anthropic API — Rate limits](https://platform.claude.com/docs/en/api/rate-limits) —— 支撑「API 计费的速率限制与月消费上限按档位自动提升、可申请调高」（官方，2026-08-15 核对）
- [HN 46549823](https://news.ycombinator.com/item?id=46549823) / [HN 47633396](https://news.ycombinator.com/item?id=47633396) —— 支撑「2026 上半年两次收紧第三方 harness」（分别对应 1 月封堵 OpenCode、4 月 4 日起 OpenClaw 转按量计费）及社区反弹规模（社区，2026）
- [HN 47963204](https://news.ycombinator.com/item?id=47963204) —— 第三方 harness 相关风控的后续社区讨论（社区，2026）
...
[^1]: [Claude Code — Legal and compliance](https://code.claude.com/docs/en/legal-and-compliance)（官方，2026-08-15 核对）：OAuth 凭证仅供订阅购买者本人普通使用，禁止第三方代路由，且可不经事先通知执行。
[^2]: [V2EX 1203227](https://v2ex.com/t/1203227)（社区，2026）：「号池才蹬了 1 天全封了」「域名邮箱注册整个域名被拉黑」，帖内共识是老实开团队版。
[^3]: [GitHub anthropics/claude-code #47509](https://github.com/anthropics/claude-code/issues/47509)（社区，2026-08-15 核对，状态 open）：团队版席位只有 Standard（1.25x Pro）和 Premium（6.25x Pro）两档，缺少对齐个人 Max 20x 的高档位。
[^4]: [Anthropic API — Rate limits](https://platform.claude.com/docs/en/api/rate-limits)（官方，2026-08-15 核对）：限额按用量档位（usage tier）定义，含每分钟请求数 / 输入输出 token 数与每月消费上限（Start 档 500 美元、Build 档 1,000 美元、Scale 档 200,000 美元），档位随用量历史自动上移，也可在 Console 的 Rate limits 页申请调高。
[^5]: 实测行为（2026-08-15）：用无效 key + 无效模型名请求 `/v1/messages`，返回 401 `authentication_error`（API key is invalid）而非模型错误——说明能看到「模型不存在」类报错时认证已通过。官方错误文档未明写校验顺序，此结论未见官方确证，服务端行为可能变化。
[^6]: 两次变更：2026 年 1 月，Anthropic 开始封堵 OpenCode 等第三方 CLI 使用订阅额度（[HN 46549823](https://news.ycombinator.com/item?id=46549823)）；2026 年 4 月 4 日起，订阅额度不再支持 OpenClaw 等第三方 harness，继续使用需转按量计费（[HN 47633396](https://news.ycombinator.com/item?id=47633396)）。均为社区转述与讨论，日期以帖内公告为准。

---

<sub>难度 高级 · 决策题 + 排错题 · 主线 Claude Code，横向 Copilot / Cursor / 云厂商托管</sub>

<sub>**时效**：官方规则与环境变量（legal-and-compliance、llm-gateway-connect、amazon-bedrock、rate-limits）已于 2026-08-15 逐项核对。**已知不确定**：社区帖里的封禁案例是当事人自述，无官方确认；风控判定口径官方不公开；「认证先于模型校验」是实测行为，未见官方文档确证。**易变**：套餐档位、限流阈值、订阅能否配合第三方工具用的条款变化很快；curl 示例里的模型 ID（claude-sonnet-4-6）会随模型迭代过期，以官方模型列表为准——ID 过期只会报未知模型名，不影响「凭证是否正常」的定性结论。涉及具体档位和参数的部分以官方页面为准；本篇的排查方法（先定性再改接入）长期有效。</sub>
