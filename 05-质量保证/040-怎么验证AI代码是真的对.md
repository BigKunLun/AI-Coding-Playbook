...
**叠几层由这段代码会不会上线决定**，但第 6 层「人读懂 diff 再 merge」在任何档位都不能省。
...
<details>
<summary>可直接抄的 prompt 示例</summary>
...
**做**：弱做法是把检查设成 `/goal` 条件 —— 有一个独立评估器在每一轮之后重新检查该条件，直到成立才罢休；强做法是配 Stop hook 当确定性闸门，配置见下面的折叠块。
...
**做**：用 `/code-review` 审当前 diff。v2.1.218 起默认丢到**后台 subagent**（独立上下文）跑；三种情况会退回前台执行 —— 已经有一个 review 在跑时再次触发、`-p` 非交互模式、设了 `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`。前台执行意味着它和写代码那轮共享上下文，对抗性打折。分辨信号：后台跑时触发后你能立刻继续输入、findings 等 review 结束才送回会话；前台跑时会话被占住直到 review 出结果。

<details>
<summary>审查目标、flag，以及"对照计划审"的 reviewer prompt</summary>
...
> 本层内容基于官方文档整理，本篇作者未实跑。适用面也窄：托管版 Code Review 处于 research preview，仅 Team / Enterprise 订阅可用，开了 Zero Data Retention 的组织不可用；单次 review 平均 20 分钟、成本 15–25 美元，走 usage credits 单独计费。**个人和小团队直接跳过这一层**，不算验证体系有洞 —— 把第 4 层的本地 `/code-review` 做实，覆盖的是同一类错。

**问**：Claude Code Review 的 finding 在你的 CI 里，是"看看而已"还是能卡住 merge？

**做**：它的 check run **永远以中性结论结束**[^2]，不会通过分支保护规则拦住 merge；想卡门禁，得自己在 CI 里读严重度 —— 先拿 check run ID，再解析严重度，两条命令与完整门禁见折叠块。
...
<details>
<summary>两条解析命令 + GitHub Actions 门禁片段（含两个容易踩的坑）</summary>

先拿 check run ID，再解析严重度：
...
AI 写代码，再让同一个 AI 给这份代码写测试，测试会天然贴着实现走：实现漏了"折扣率不得超过 0.5"这条需求，接着写的测试就只断言正常折扣算得对，同样不碰上限 —— 绿灯，但需求根本没被断言过。所以第 1 层的证据标准不是"测试绿了"，而是"每条需求都能指到一个 assert"。换成覆盖率也一样。覆盖率只记录"这行被执行到了"，不检查"执行结果被断言过"，上面那种贴着实现写的测试照样能刷出 100% 行覆盖。Meta 在生产代码上给出过一个更硬的数字：他们用变异测试驱动生成的 571 个加固测试里，277 个（约 49%）不增加任何行覆盖率，但它们抓得到现有测试抓不到的故障[^6]。也就是说，只拿覆盖率当判据，会漏掉近一半真正有价值的测试。想在覆盖率之外加一个判据，用变异测试：工具自动改坏代码（把 `>=` 改成 `>`、把返回值取反），看你的测试能不能挂掉，挂掉的比例就是 mutation score。Python 用 mutmut。个人研究结论、未经外部确证：核心业务模块把 mutation score 的基线定在 80%，每月体检一次，存活的变异体丢给 AI 让它补测试。反过来，一旦给 agent 一个能产出通过或失败的东西，这个循环会自己闭合[^3] —— 这就是断言层存在的理由：它把"看起来做完了"换成机器可读的对错。

### 多层同时被穿过一次，但那不说明层数无效

官方 4-23 postmortem 记录：一个让 Claude 变"健忘"的 bug，通过了多轮人工和自动代码审查，也通过了单元测试、端到端测试、自动化验证和内部试用[^4]。按本篇的分层框架对照，第 1、3、4、6 层对应的机制全部被穿透 —— 这是本篇的对照解读，非原文表述；postmortem 没提第 2 层（类型约束）和第 5 层（CI 严重度门禁）是否存在。那还叠个什么劲？三点：

1. **每层挡的是不同类的错，不是同一类的重复投票。** 类型系统挡幻觉 API，测试挡行为回归，人审挡业务意图错。漏掉那个 bug 说明已叠的那几层里没有一层的职责范围覆盖它，不说明层数无效。
...
AI 的产出速度远超验证速度：一个人每天要看 20 个 PR、50-60 个分散文件，改动是中间三行、这里五行、结尾又四行，根本攒不出整文件的上下文。社区把这叫"验证复杂度壁垒" —— 组件越多，验证它们能否协同工作所需的时间超线性增长。这跟"责任在人"不矛盾。恰恰因为最终背锅的是签字的那个人，盖橡皮图章才是整个体系里最危险的失效模式。所以第 6 层的判定标准写成了"能不能用自己的话写出三条答案"，而不是"读没读过"。
...
| 工具 | 验证体系对应物 | 差异 |
|------|--------------|------|
| Claude Code | Code Review（托管，多 agent）+ 本地 `/code-review` + Stop hook | 同时有"多 agent 机审"和"能阻止回合结束的 Stop 闸门"；托管版需 Team / Enterprise |
| Cursor | Cursor Bot review + hooks | 同属机审；有 hooks 机制，但其 stop hook 是通知型、按官方文档不能阻止回合结束 |
| GitHub Copilot | Copilot review + hooks | 同属机审；agentStop hook 同为通知型，官方文档只给 preToolUse 拦截能力 |
...
- [Code Review — Claude Code Docs](https://code.claude.com/docs/en/code-review) —— 支撑第 4、5 层：research preview 与 Team/Enterprise/ZDR 限制、"默认只关注正确性"、中性结论、`bughunter-severity` 两条命令与返回格式、单次平均 20 分钟与 15–25 美元且走 usage credits、`/code-review` 的 flag 与 v2.1.218 起默认后台 subagent 及三种前台回退条件（官方，2026-08-15 核实）
- [Hooks reference — Claude Code Docs](https://code.claude.com/docs/en/hooks) —— 支撑第 3 层：Stop hook 的 settings.json 结构、exit 2 阻断（官方，2026-08-15 核实）
- [Automate actions with hooks — Claude Code Docs](https://code.claude.com/docs/en/hooks-guide) —— 支撑"连续 8 次 block 后被覆盖"及上限可调（官方，2026-08-15 核实）
...
- [ACH: Mutation-Guided LLM-based Test Generation at Meta — arXiv:2501.12862](https://arxiv.org/abs/2501.12862) —— 支撑"49% 的加固测试不增加行覆盖率"这条覆盖率局限证据（学术/工业报告，FSE 2025 Industry Track，2026-08-15 对照论文摘要核实：571 个测试中 277 个在只按行覆盖率筛选时会被丢弃）
...
[^1]: [Automate actions with hooks — Claude Code Docs](https://code.claude.com/docs/en/hooks-guide)（官方，2026-08-15 核实）：Stop hook 连续 block 8 次且无进展后，Claude Code 会覆盖 hook 并结束回合；上限可用 `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` 调整。
[^2]: [Code Review — Claude Code Docs](https://code.claude.com/docs/en/code-review)（官方，2026-08-15 核实）：check run 总以中性结论结束，因此不会通过分支保护规则阻塞合并。
...
[^6]: [ACH: Mutation-Guided LLM-based Test Generation at Meta — arXiv:2501.12862](https://arxiv.org/abs/2501.12862)（学术/工业报告，2025，2026-08-15 对照摘要核实）：571 个加固测试中 277 个（约 49%）在只按行覆盖率这一充分性判据筛选时会被丢弃，即不增加行覆盖率。

---

<sub>难度 中级 · 流程题 · 主线 Claude Code，横向 Cursor / Copilot</sub>

<sub>**时效**：命令、配置片段与产品状态（Code Review 处于 research preview、仅 Team/Enterprise 可用且 ZDR 组织不可用、check run 恒为中性结论、单次平均 20 分钟与 15–25 美元且走 usage credits、`gh api` 解析 `bughunter-severity` 的写法与返回格式、`/code-review` v2.1.218 起默认后台 subagent 及三种前台回退条件、Stop hook 的 settings.json 结构与连续 8 次 block 上限）已于 2026-08-15 对照官方文档逐条核实。**已知不确定**：第 5 层（托管 Code Review 的 CI 门禁）全部基于官方文档整理，本篇作者未实跑，命令与门禁片段未经真实 PR 验证；mutation score > 80% 的基线出自个人研究，未经外部确证，Meta 论文本身没有给出这个阈值。"Opus 4.7 抓到、Opus 4.6 漏掉"出自 2026-04 postmortem，是当时代际的快照，只作为"跨代 back-test 有效"的方法论证据。**易变**：托管 Code Review 的可用范围与定价、`/code-review` 的 flag 与后台行为随版本调整；横向对比里"Cursor / Copilot 的 stop 类 hook 均为通知型、不能阻止回合结束"是 2026-08-15 对照两家官方文档的快照，两家 hooks 能力更新很快，用前回查。</sub>
