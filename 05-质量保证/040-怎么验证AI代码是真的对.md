...
**一句话**：验证不是"人读一遍"，是断言、约束、运行、对抗机审、CI 门禁、人这六层分工 —— 按场景风险决定叠几层，但最后签字的永远是人。
...
| 机审报了 Important，PR 照样能 merge | 在 CI 里解析 `bughunter-severity` 自己卡 | 机审 check run 恒为中性结论 |
...
六层自查清单。每层先答一个是/否问题；答"否"就照做最小动作，再用"证据"那行确认真的做对了。全答"是"，说明没有结构性缺口。

### 第 1 层｜断言：agent 有没有一个自己能跑的通过/失败信号

**问**：你交给 Claude 的任务里，有没有一条它自己能跑、能读到结果的命令（测试 / build / lint）？

**做**：把验证条件写进 prompt，让它跑完再停。
...
**前提**：托管版 Code Review 处于 research preview，仅 Team / Enterprise 订阅可用，开了 Zero Data Retention 的组织不可用；其他 plan 只能用本地 `/code-review`。单次 review 平均约 20 分钟、成本约 15–25 美元，走 usage credits 单独计费。
...
<sub>**时效**：命令、配置片段与产品状态（Code Review 处于 research preview、仅 Team/Enterprise 可用、check run 恒为中性结论、`gh api` 解析 `bughunter-severity` 的写法、Stop hook 的 settings.json 结构）已于 2026-08-03 对照官方文档逐条核实。**已知不确定**："Opus 4.7 抓到、Opus 4.6 漏掉"出自 2026-04 postmortem，是当时代际的快照，只作为"跨代 back-test 有效"的方法论证据。**易变**：托管 Code Review 的可用范围与定价、`/code-review` 的 flag、`--worktree` 类 CLI 行为随版本调整，用前回查官方文档。</sub>
