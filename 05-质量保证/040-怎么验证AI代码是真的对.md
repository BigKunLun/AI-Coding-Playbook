...
一句话切边界：某一层的深用见对应指针——人审看 [#041](./041-怎么审查AI生成的代码.md)、幻觉 API 查防看 [#042](./042-幻觉API与幻觉依赖.md)、改不对何时停手看 [#043](./043-fix-loop什么时候必须停.md)、TDD 看 [#030](../04-执行工作流/030-AI写的测试不可信.md)。
...
**自查动作**：把测试和需求并排放，逐条问"这条需求对应哪个 assert"。对不上的就是缺口。TDD 的做法是先写会失败的测试，见 [#030](../04-执行工作流/030-AI写的测试不可信.md)。
...
|--------|-----------|---------|------|--------|---------|
| **人（最终负责）** | 读 diff、设门禁、最终拍板 | 业务意图错、需求理解偏、跨系统副作用 | 规模化产出下会盖橡皮图章 | 任何生产代码 merge 前 | [#041](./041-怎么审查AI生成的代码.md) |
| **机审（同上下文 / 同模型）** | Claude Code Review、本地 `/code-review`、静态分析器 | 逻辑错、安全漏洞、边界 case、回归 | 模型盲区和实现一致；上下文不全就漏 | 任何 PR、commit 前 | [#041](./041-怎么审查AI生成的代码.md) |
| **对抗机（独立视角 / 跨模型）** | 独立上下文的 reviewer SubAgent、跨模型互查 | 同模型盲区、确认偏差（写代码的 agent 倾向说自己写得好） | 跨模型仍可能共享训练盲区 | 生产 / 关键路径；怀疑自评过于乐观时 | [#032](../04-执行工作流/032-SubAgent并行开几个.md) |
| **自动化（测试 + 运行 + CI）** | TDD 闭合环、build / lint / 截图对比、CI 卡门禁 | 行为正确性、回归、构建 / 类型错 | AI 写的测试可能同义反复；只能验证"你想到要测的" | 任何代码（最低验证基线） | [#030](../04-执行工作流/030-AI写的测试不可信.md) |
...
| 手段 | 一句话定位 | 信号类型 | 指针 |
|------|-----------|---------|------|
| **读（人审）** | 人读 diff、读懂了才 merge | 主观判断、业务意图 | [#041](./041-怎么审查AI生成的代码.md) |
| **断言（测试）** | 给 agent 一个通过 / 失败信号，让它自己闭合验证环 | 机器可读的"对 / 错" | [#030](../04-执行工作流/030-AI写的测试不可信.md) |
| **约束（类型 / lint / schema）** | 编译器、类型系统、OpenAPI schema 作为独立于测试和实现的真值源，专门挡 AI 编出来的 API 和依赖 | 编译期 / 契约错、幻觉 API 或包 | [#042](./042-幻觉API与幻觉依赖.md) |
...
深用见 [#030](../04-执行工作流/030-AI写的测试不可信.md)。
...
报错 = 这层活着；静默通过 = 你的类型检查没覆盖到这里。幻觉查防的完整手法见 [#042](./042-幻觉API与幻觉依赖.md)。
...
⚠️ 官方提醒：被要求找缺口的 reviewer 通常总能找出点什么，追着每条改会导致过度设计——明确告诉它只报影响正确性或既定需求的缺口✅。机制见 [#032](../04-执行工作流/032-SubAgent并行开几个.md)。
...
人审的具体技巧（REVIEW.md、quality rubric、对抗式 reviewer persona）见 [#041](./041-怎么审查AI生成的代码.md)。

---

> **怎么用这张清单**：原型只查第 1、6 层；内部工具查 1、2、4、6；生产 / 安全敏感六层全查。

> **多 agent 分工的社区模式**（指针，非本篇攻略）：turlockmike 的"乒乓结对编程"——第一个 agent 对着 spec 写一个最小的失败测试，第二个 agent 只写刚好能让它通过的最小实现，如此往复（[@turlockmike](https://news.ycombinator.com/item?id=47234917)）✅。机制归 [#032](../04-执行工作流/032-SubAgent并行开几个.md)。
...
核心差异在"对抗机审 + 跨模型"这一层。Claude Code 原生支持多 agent 编队（官方称"一队专门的 agent"），而跨模型互查目前还得靠人手动复制粘贴——社区做法是"把 Claude 的代码复制粘贴给 ChatGPT 和 Gemini，让它们互相检查"（[@lateforwork](https://news.ycombinator.com/item?id=47234917)）✅。机审工具的详细对比见 [#041](./041-怎么审查AI生成的代码.md)。
...
- ❌ **验证一直不过、越改越糟还硬撑。** 这是调试侧的问题，见 [#043](./043-fix-loop什么时候必须停.md)。
...
**交叉引用：**
- [#004 Claude Code 的 plan / skill / subagent / CLAUDE.md 这堆"能力"到底是啥、什么时候用哪个？](../01-心智与工具/004-Claude-Code能力地图.md) —— "验证闭合层"的一句话定位（本篇是它的深入）。
- [#030 TDD 在 AI coding 里怎么做？](../04-执行工作流/030-AI写的测试不可信.md) —— 自查清单第 1 层的攻略。
- [#031 plan → execute → review 的正确循环怎么做？](../04-执行工作流/031-plan-execute-review循环.md) —— 本篇的验证体系是 review 环的体系化。
- [#032 开几个 agent 并行跑，结果互相踩、被限流、token 指数级炸开——并行到底该开几个？](../04-执行工作流/032-SubAgent并行开几个.md) —— 自查清单第 4 层的机制。
- [#041 怎么 review AI 生成的代码？（人审 + 机审策略）](./041-怎么审查AI生成的代码.md) —— 自查清单第 6 层的攻略、REVIEW.md 模板。
- [#042 AI 写的方法不存在、pip install 报 404、API 参数对不上——幻觉 API 和幻觉依赖怎么查、怎么防？](./042-幻觉API与幻觉依赖.md) —— 自查清单第 2 层的攻略。
- [#043 改不对、越改越糟，什么时候该停手？](./043-fix-loop什么时候必须停.md) —— 验证一直不过时怎么跳出 fix loop。
