...
[#040](./040-怎么验证AI代码是真的对.md) 已经讲过：验证是一个多层体系，人审只是其中负责“拍板”的一环，任务是读 diff、设门禁、做最终决定。
...
一句话划边界：机审是分流的闸门，不是最终裁判，拍板永远在人。体系全景见 [#040](./040-怎么验证AI代码是真的对.md)，测试相关归 [#030](../04-执行工作流/030-AI写的测试不可信.md)，幻觉 API 的查防见 [#042](./042-幻觉API与幻觉依赖.md)，改不对时何时停手见 [#043](./043-fix-loop什么时候必须停.md)。
...
tonybai 把这总结为“审查剧本，而不是审查表演”：在开发者大规模生成代码前，先审查其核心设计思路、任务分解和关键 prompt。来源见 [tonybai：AI 代码审查的危与机](https://tonybai.com/2025/12/27/code-review-hell-in-ai-age/)（社区权威，2025-12）。计划-执行-审查这个循环本身见 [#031](../04-执行工作流/031-plan-execute-review循环.md)。
...
3. 重点看被删掉的代码。agent 删测试、删边界检查是常见的偷懒方式，详见 [#030](../04-执行工作流/030-AI写的测试不可信.md) 的 TDD 攻略。
4. 关注新增的 API 和依赖。AI 编造出不存在的包或 API 是一类高发错误，查防手法见 [#042](./042-幻觉API与幻觉依赖.md)。
...
派一个独立上下文的 reviewer 子 agent，只给它 diff 加验收标准，让它挑刺。子 agent 的机制归 [#032](../04-执行工作流/032-SubAgent并行开几个.md)。关键是：写代码的 agent 不能等于评分的 agent。
...
两个 agent 互不共享上下文，机制指针见 [#032](../04-执行工作流/032-SubAgent并行开几个.md)。
...
- ❌ **review 发现问题、让 agent 改、越改越糟还硬撑**。这是另一个问题，属于调试侧何时停手，见 [#043](./043-fix-loop什么时候必须停.md)，不是 review 策略本身的问题。
...
**交叉引用：**
- [#040 AI 说"做完了"、测试也全绿，怎么知道代码是真的对？](./040-怎么验证AI代码是真的对.md) —— 验证体系总览，本篇是其中“人审”一环的执行层面深入，必引。
- [#030 TDD 在 AI coding 里怎么做？](../04-执行工作流/030-AI写的测试不可信.md) —— 测试作为验证手段的攻略，本篇不碰 TDD 的红绿循环。
- [#031 plan → execute → review 的正确循环怎么做？](../04-执行工作流/031-plan-execute-review循环.md) —— 本篇 self-review 段“先读计划”对应的循环指针。
- [#032 开几个 agent 并行跑，结果互相踩、被限流、token 指数级炸开——并行到底该开几个？](../04-执行工作流/032-SubAgent并行开几个.md) —— 本篇对抗机审段“独立上下文 reviewer 子 agent、乒乓式结对”的机制指针，必引。
- [#042 AI 写的方法不存在、pip install 报 404、API 参数对不上——幻觉 API 和幻觉依赖怎么查、怎么防？](./042-幻觉API与幻觉依赖.md) —— 本篇 self-review 段“新 API、新依赖”一项的攻略指针（类型、lint、schema 手法）。
- [#043 AI 改不对 bug、陷入 fix loop / 越改越糟，什么时候必须停、怎么跳出？](./043-fix-loop什么时候必须停.md) —— 本篇反模式段的指针，讲跳出改不动的循环。
- [#053 AI 生成的 PR 太大太多，review 机制怎么扛住不崩？](../06-工程协作/053-AI的PR太大没法review.md) —— 本篇讲单个 PR 怎么审，053 讲 PR 又大又多时 review 机制怎么扛住。
