...
**证据**：仓库里出现 `*-snapshots/` 目录且有 PNG 文件；故意把某个按钮的 `padding` 加 20px 再跑，测试应当变红，`test-results/` 里出现 expected / actual / diff 三张图。三张图没出现，说明你截的是 `page.screenshot()` 而不是 `toHaveScreenshot`，前者不做对比。

预期内的视觉改动用 `npx playwright test --update-snapshots` 更新基线，并把新基线放在**同一个 PR** 里，让 review 的人同时看到代码 diff 和图 diff。
...
截图看得见"长歪了"，看不见"点了没反应"。控制台是成本最低的第二个信号 —— 它是机器可读的数字，不是主观判断。
...
不写测试也能验：Claude Code 接上 Chrome 之后可以直接读控制台和 DOM 状态，然后去修造成报错的代码[^2]。给它的话要指明**找什么模式**，别让它把全部日志倒出来 —— 官方文档专门提醒日志可能很啰嗦[^3]：

```text
用浏览器打开 localhost:3000/checkout，点"提交订单"，
只把控制台里 error 级别的条目原样贴出来，一条都没有就明确说 0 条。
```

<details>
<summary>Claude Code 连浏览器的前置条件（2026-08 核实）</summary>
...
plan mode 里的权限分档：`read_page`、`get_page_text`、`find`、读控制台和网络请求、截图属于只读，不弹权限；点击、输入、导航、标签页管理、录 GIF 会弹。v2.1.199 起，只读调用一旦带上改状态的参数（截图的 `save_to_disk`、控制台读取器的 `clear`）也会弹。
...
前两个信号只能判"变了没有"和"报错没有"，判不了"是不是设计想要的那样"。这一步必须人来，但反馈方式决定了 AI 能不能一次改对。

做法：截图 → 在图上圈出问题处 → 把图连同**元素定位 + 数值差异**一起给 Claude Code：
...
前三步只要靠"我记得去看"，就一定会漏。把它变成和代码测试同一条门禁：
...
然后在 `CLAUDE.md` 的 workflow 区写死一条：
...
更硬的做法是 Stop hook —— 回合结束前自动跑 `verify:ui`，不过就 `exit 2` 拦住它，配置模板直接复用 [#040](./040-怎么验证AI代码是真的对.md) 第 3 层的折叠块。

**证据**：故意改坏一处样式，让 Claude 随便改点别的并结束回合，应当看到它没能停下、而是回头去修视觉测试。它照常结束了，说明脚本没执行权限或路径写错。

## 为什么

### 像素 diff 交给算法，语义判断才交给模型

Playwright 的视觉对比走的是确定性的像素比对，超过 `maxDiffPixels` 就红，同一份输入永远同一个结论[^1]。模型看图不是这样 —— 它能发现"文字溢出了""图片没加载"这类明显异常，但 2px 的偏移、轻微的色相变化，它给不出稳定结论（社区经验，未见官方确证）。

所以分工是：**算法负责"变没变"，模型负责"这个变化要不要紧"**。反过来用（让模型逐张比对像素）就会得到一个每次结论都不一样的门禁，还不如没有。

### 控制台 error 是唯一不需要写断言就存在的信号

写视觉基线要成本，写交互测试要成本，但控制台 error 是浏览器白送的 —— 页面一跑就有。它的覆盖面也刚好补上截图的盲区：截图看不见 `Uncaught TypeError`，看不见接口 404，看不见 React 的 key 警告升级成的渲染异常。

代价是它只能报"确实炸了"，报不了"逻辑错了"。所以它是门槛，不是终点 —— [#040](./040-怎么验证AI代码是真的对.md) 的六层里，它属于第 1 层断言在 UI 场景的最低配版本。
...
- [Use Claude Code with Chrome — Claude Code Docs](https://code.claude.com/docs/en/chrome) —— 支撑第 2 步全部前置条件、plan mode 权限分档、"默认开启增加上下文占用"、"日志啰嗦，告诉它找什么模式"（官方，2026-08 核实）
- [Visual comparisons — Playwright Docs](https://playwright.dev/docs/test-snapshots) —— 支撑 `toHaveScreenshot` 首次生成基线、`maxDiffPixels`、`stylePath`、`--update-snapshots`（官方，2026-08 核实）
- [Hooks reference — Claude Code Docs](https://code.claude.com/docs/en/hooks) —— 支撑第 4 步 Stop hook 的 `exit 2` 阻断（官方，2026-08 核实）

[^1]: [Visual comparisons — Playwright Docs](https://playwright.dev/docs/test-snapshots)（官方，2026-08 核实）：首次运行自动生成参考截图并保存；`maxDiffPixels` 控制允许的像素差异数，可按用例或全局配置。
[^2]: [Use Claude Code with Chrome — Claude Code Docs](https://code.claude.com/docs/en/chrome)（官方，2026-08 核实）：连上 Chrome 后可直接读取控制台错误与 DOM 状态并去修造成问题的代码；需扩展 1.0.36+、直连 Anthropic 付费计划、不支持 WSL 与第三方渠道。
[^3]: [Use Claude Code with Chrome — Claude Code Docs](https://code.claude.com/docs/en/chrome)（官方，2026-08 核实）：日志可能非常啰嗦，应告诉 Claude 找什么模式，而不是让它输出全部控制台内容。

---

<sub>难度 中级 · 流程题 · 主线 Claude Code，横向 Playwright / Playwright MCP</sub>

<sub>**时效**：Chrome 集成的前置条件（扩展 1.0.36+、必须 `/login`、Bedrock/Foundry 不可用、WSL 不支持、v2.1.211 之前 `save_to_disk` 不写文件、v2.1.199 起只读调用带改状态参数会弹权限）与 Playwright 的 `toHaveScreenshot` / `maxDiffPixels` / `stylePath` / `--update-snapshots` 已于 2026-08-04 对照官方文档核实。**已知不确定**：`maxDiffPixelRatio`、`threshold`、`mask`、`animations` 等选项在本次核实的官方页面上未出现，用前查 Playwright API reference；"模型看图测不准 2px 偏移""跨平台字体渲染让 diff 恒红""组件级截图比整页稳"均为社区经验，未见官方确证。**易变**：Chrome 集成的计划限制与版本行为随版本调整，用前跑 `/chrome` 看状态面板回查。</sub>
