...
registry 返回 200、下载量也正常，只说明这个包**存在且不是抢注**，不说明**该引**。AI 推荐依赖有个稳定的偏好：能引包就引包，哪怕这活儿自己写十几行就完了。所以在"存在性 → 抢注"之后再加一道"必要性"闸门，三条全过才装：

| 门槛 | 怎么判 | 不过的处理 |
|---|---|---|
| **一句话说清它干什么** | 让 AI 用一句话写出"这个包替我们解决了什么，不引会怎样"。写不出边界、只说"通用工具库""各种辅助函数"，就是凑数 | 退回，要它改成自己写 |
| **最近一次发布在 6 个月内** | 查 registry 的最新版本发布时间（命令见折叠块） | 超期先去看 GitHub 有没有 issue 积压和"寻找维护者"公告，再决定 |
...
<details>
<summary>查"最近一次发布"的命令 + 实测输出</summary>
...
<sub>**时效**：命令与工具行为于 2026-08-03 实测复核 —— Context7 现行工具名为 `resolve-library-id` + `query-docs`（旧名 `get-library-docs` 已更名）；`import/no-unresolved` 能挡幻觉包而 Ruff 没有等效规则；`pip index versions` 的 experimental 警告已在 pip 25.1（2025-04-26）移除，但输出走 stderr。**已知不确定**：diffray 的"RAG 降幻觉 60-80%"来自厂商博客转引，无独立复现，只作量级参考。**已知不确定（补）**：③ 的三道门槛里，"6 个月内有发布"是社区常用的维护活跃度分界，未见官方确证；"30 行内自实现"与"20 行以下任务不引依赖"是个人经验取值，无外部依据，团队可自行调整。**易变**：MCP 工具名是本篇最易变的一类信息，报 `tool not found` 时用 `/mcp` 查实际名；registry 返回字段与 pyright 版本号会随时间变；③ 的 PyPI / npm 查询命令于 2026-08-04 用 `requests`、`left-pad` 实跑验证。"幻觉类型 × 查防手段"矩阵不依赖版本，长期有效。</sub>
