...
| AI 推荐了一个你没听过的包，registry 一切正常 | 过必要性三道门槛：说得清 / 6 个月内有发布 / 30 行内自实现不了 | 20 行以内的任务一律不引新依赖 |
...
flowchart TD
    ...
    D --> G{状态码}
    G -->|404| H[编的，别装]
    G -->|200| I{首发几周内<br/>且周下载三位数}
    I -->|是| J[疑似抢注<br/>别装]
    I -->|否| K[再过必要性三道门槛]
```

**类型检查先分出幻觉包还是幻觉 API**；registry 返 200 只证明存在，还要过抢注和必要性两关。

## 怎么做

先把幻觉分成两类，因为查法完全不同：**幻觉包**是 AI 让你装的依赖根本不存在，**幻觉 API** 是包真、但方法名 / 参数 / 签名 / 版本错。六种手段各挡哪类、怎么配，按折叠块里的矩阵对号入座；下面 ①-⑥ 只给幻觉专项配置。

<details>
<summary>「幻觉类型 × 查防手段」矩阵 + 各手段的归属篇</summary>
...
每种手段的完整攻略归对应篇：约束层的体系定位见 [#040](./040-怎么验证AI代码是真的对.md)，运行验证归 [#030](../04-执行工作流/030-AI写的测试不可信.md)，REVIEW.md 通用模板归 [#041](./041-怎么审查AI生成的代码.md)。

</details>

### ① 类型系统：第一道闸，也是最能分类的一道

TypeScript 开 `strict: true` 加 `noUncheckedIndexedAccess`；Python 用 `pyright` 或 `mypy --strict`，pyright 的 `reportMissingImports` 默认就是 `"error"`，显式写进 `pyproject.toml` 防被人调低[^1]；HTTP API 幻觉（AI 编的 endpoint 或字段）用 OpenAPI schema 挡，`openapi-typescript` 从 spec 生成类型。

**怎么确认这层活着**：见折叠块的一开一关自测 —— 光跑一次看到报错不算数，那可能只是撞上了默认值。mypy 用户注意折叠块里的 `ignore_missing_imports` 坑。

<details>
<summary>eslint 与 pyright 的最小配置 + 一开一关自测 + mypy 的坑</summary>
...
自测文件必须建在**项目目录里**：eslint 的 flat config 是从被检查文件所在目录向上查找的，检查 `/tmp` 下的文件会找不到项目里的 `eslint.config.js`，直接报 `Could not find config file`，测的就不是这套规则。

```bash
printf "import x from 'totally-not-a-real-pkg-42';\nconsole.log(x);\n" > src/__halluc_test.js
npx eslint src/__halluc_test.js
# 预期出现：error  Unable to resolve path to module 'totally-not-a-real-pkg-42'  import/no-unresolved
rm src/__halluc_test.js   # 测完删掉
```

2026-08 实测：上面这条报错原样出现，退出码 1。这行没出现说明规则没生效（最常见原因是 eslint 没匹配到这个文件，或 resolver 没配好），你以为的这道闸是空的。

用 mypy 的话有个坑：配置里开了 `ignore_missing_imports = True`（很多老项目为了消噪都开了），`Cannot find implementation` 这条报错会被整个吞掉，幻觉包静默通过。用 mypy 挡幻觉前先确认这个开关是关的。
...
**光有"存在"还不够 —— 被抢注的包也是 200。** 幻觉包名可以被预测性抢注（slopsquatting），所以对 AI 推荐的每个新包还要看首发时间和下载量：首发在最近几周内 + 周下载量三位数以内 + 只有 0.0.1 一个版本 + GitHub 仓库空或不存在，同时出现就当疑似抢注处理，别装。命令、对照样例、以及为什么 `npm audit` / `pip-audit` 这类 SCA 工具挡不住这一类，见折叠块。

<details>
<summary>查首发时间与下载量的命令、"正常包长什么样"的对照、SCA 为什么挡不住</summary>

2026-08-15 实测状态码：`langchain-openai-agent` → `404`；`langchain-openai` → `200`。
...
2026-08-15 跑 `langchain-openai` 的实际输出，可以当"正常包长什么样"的对照：

```
首次发布: 2024-01-05T23:12:33
最新版本: 1.5.1
{"data":{"last_day":3142068,"last_month":69888923,"last_week":17699607},...}
```

反向判定：几年历史 + 周下载量六位数以上，基本可以排除 slopsquatting（不排除该包本身有 CVE，那是 SCA 的事）。

**SCA 工具在挡幻觉这件事上帮不上忙。** `npm audit` / `pip-audit` / Snyk 的输入是**已经声明或已经装上**的依赖树，比对的是已知漏洞库[^3]。所以：AI 只在源码片段里写了一行 `import fake_pkg`、还没进 manifest，SCA 看不到；幻觉包不存在，不在漏洞库里；抢注包是新注册的，还没被收录成 CVE。挡源码那行 import 的是 `import/no-unresolved` / pyright，挡"装进来"的是 lockfile + registry 查询。
...
registry 返回 200、下载量也正常，只说明这个包**存在且不是抢注**，不说明**该引**。AI 推荐依赖有个稳定的偏好：能引包就引包，哪怕这活儿自己写十几行就完了 —— 这条偏好是个人与社区的普遍观察，未见系统性测量。所以在"存在性 → 抢注"之后再加一道"必要性"闸门，三条全过才装：**一句话说清它干什么、最近一次发布在 6 个月内、30 行内自实现不了**。每条怎么判、不过怎么处理，见折叠块。

再加一条硬规则：**任务本身 20 行以内写得完，一律不引新依赖。** 这条比上面三条更容易执行，因为不用判断，只看规模。

<details>
<summary>三道门槛的判法、查"最近一次发布"的命令 + 实测输出</summary>

三个数字都是经验值，不是官方阈值：6 个月是社区常用的"仍在维护"分界线，社区经验，未见官方确证；30 行、20 行是小C 的个人取值，团队可以按自己的代码规范调，重点是**先定死一个数，别每次现场讨论**。

| 门槛 | 怎么判 | 不过的处理 |
|---|---|---|
| **一句话说清它干什么** | 让 AI 用一句话写出"这个包替我们解决了什么，不引会怎样"。写不出边界、只说"通用工具库""各种辅助函数"，就是凑数 | 退回，要它改成自己写 |
| **最近一次发布在 6 个月内** | 查 registry 的最新版本发布时间（命令见下） | 超期先去看 GitHub 有没有 issue 积压和"寻找维护者"公告，再决定 |
| **30 行内能否自实现** | 让 AI 先给一份不引这个包的实现，看行数 | 30 行内写得出，就用自己的实现 |

```bash
# PyPI：最新版本 + 该版本的发布时间
curl -s https://pypi.org/pypi/<pkg>/json | python3 -c "import json,sys;d=json.load(sys.stdin);v=d['info']['version'];print('最新版本:',v);print('最近发布:',max(f['upload_time'] for f in d['releases'][v]))"

# npm：最新版本 + 最近一次改动时间
npm view <pkg> dist-tags.latest time.modified
```

2026-08 实测，一正一反的对照：

```
# requests           最新版本: 2.34.2   最近发布: 2026-05-14   → 3 个月内，过
# npm view left-pad  latest = 1.3.0     time.modified = 2024-04-16  → 超 2 年，不过
```

注意 ② 那节查的是 `time.created`（首发时间，用来识别抢注），这里查的是最近一次发布时间（用来识别废弃）。两个字段方向相反，别搞混：新得可疑是抢注，旧得可疑是没人维护。

把这三道门槛写进 `CLAUDE.md` 能让 agent 自己先过一遍：

```markdown
引入新依赖前必须在回复里逐条写出：(1) 一句话说明它解决什么；
(2) 最近一次发布日期，超过 6 个月要说明为什么仍然可用；
(3) 不引它的自实现大概多少行。任务本身 20 行内写得完，直接自实现，不引依赖。
```

**怎么确认生效**：下一次让 agent 加功能时，它应在装包前主动列出这三条；直接跑 `pip install` 就说明规则没被读到，排查见 [#010](../02-上下文工程/010-CLAUDE-md怎么写才生效.md)。

</details>

### ④ 让 agent 查官方文档，而不是查自己的记忆

让 agent 用 Context7 MCP：先 `resolve-library-id` 查包名，再 `query-docs` 查这个 API 在当前版本的签名。这一步把幻觉从"概率脑补"变成"检索核对"。两条纪律：

- **工具名会变，先兜底。** `resolve-library-id` + `query-docs` 是 2026-08 对照官方 README 核到的名字（v2.0.0 起从 `get-library-docs` 改名而来）。agent 报 `tool not found` 时，在 Claude Code 里跑 `/mcp` 列出当前已连接 server 实际暴露的工具名，按实际名字改口，别照着本文硬试。
- **查文档时带上 lockfile 里的版本号问**，比如"在 langchain-openai 1.5.1 里，ChatOpenAI 的构造参数有哪些"。不带版本号，检索回来的可能是另一个大版本的文档，等于换了个来源继续幻觉；人审扫到新 API 时同理，去官方文档核对签名、参数、版本，别信自己对 API 的记忆 —— AI 训练数据里混着多个版本，用错版本的 API 是高发错误。

### ⑤ 最后 build 一次，让它现形

给 agent 一个 build / import / 测试当作"通过或失败"的信号，幻觉会在这一步暴露[^4]：ImportError、404 或编译报错。agent 说"我用了某某 API"时，要它贴出 import 或 build 的输出，而不是宣告。这一层是兜底也是最硬的一层：幻觉 API 的语法通常是对的，静态分析和编译都可能过，只有跑到那条代码路径上才现形 —— 而自动化测试未必覆盖到那条路径。所以它和 ① 是互补关系，不能只留一个。

### ⑥ 机审侧的两条幻觉专项规则

REVIEW.md 里加两段幻觉专项（完整片段见折叠块，通用七 pattern 模板见 [#041](./041-怎么审查AI生成的代码.md)）：API 存在性断言必须引官方文档 URL 或本地类型定义的 `file:line`，不许从函数名推断；新依赖必须出现在 lockfile 里，否则标 Important、装前人工查 registry。

**怎么确认生效**：跑 `/code-review` 后，每条关于 API 存在性的 finding 都应带 `file:line` 或官方文档 URL。某条只写"这个 API 看起来不存在"而没有引用，就是靠命名推断，退回重查。

<details>
<summary>REVIEW.md 幻觉专项片段（可直接复制）</summary>
...
一项学术研究用 16 个主流代码 LLM 生成了 576,000 个代码样本（Python + JavaScript），推荐出的 223 万个包里有 440,445 个（19.7%）是幻觉，其中 205,474 个是唯一的不存在包名[^5]。**这个 19.7% 是把大量开源模型样本混进来的均值**，论文摘要给的分档是商业模型至少 5.2%、开源模型至少 21.7% —— 你用 Claude / GPT 对标的是 5.2% 那档，别拿 19.7% 吓自己，也别因为"我用的是好模型"就不查。

真正危险的是第二个数字：**同一个 prompt 跑 10 次，同一个幻觉包重复出现超过一次的比例是 58%**。重复就意味着可预测，攻击者能提前把这些不存在的包名注册到公共仓库里塞进恶意代码 —— 这就是 slopsquatting，它让幻觉从"跑不通"升级成一条供应链攻击向量。还有个反直觉的数字：全部包名去重后约 81% 只由单个模型生成过，且真实包的跨模型共享度更高、幻觉包更偏单模型独有 —— 换模型不能消除幻觉，只会撞出另一批。

### AI 为什么会编：概率 + 训练激励的双重结果

直接机制是社区的"西瓜树"比喻：语法 token 有大量训练数据保障所以几乎不错，具体 API 的 token 在数据缺失时就按概率塌成了最像的那个词；再叠一层训练激励 —— 模型被优化成"自信的考生"，评测分不清错误陈述和事实时，自信地猜比承认不确定更划算。

所以幻觉是结构性产物，不能靠"AI 应该知道"来防，必须靠外部真值源来查 —— 类型定义、依赖清单、官方文档、实际运行，正好对应上面的几层手段。

### 好消息：幻觉是会现形的那一类错

代码里的幻觉在 LLM 的错误里算相对好抓的：一运行，幻觉方法立刻报错现形，区别于会静默通过的逻辑错和安全漏洞。这也是本篇整套方法能成立的基础 —— 幻觉有确定性的暴露点（类型检查、install、build、import），所以能配成流水线上的闸门，而不是靠人盯。
...
<details>
<summary>参考资料（17 条）</summary>

- [arXiv:2406.10279 — We Have a Package for You!](https://arxiv.org/abs/2406.10279) —— 支撑"为什么"第 1 节的全部数字：16 个 LLM × 576,000 样本 × 223 万推荐包 × 19.7% 幻觉、商业 5.2% / 开源 21.7% 的分档（摘要）、58% 重复率（§5.3）、81% 单模型独有（§5.4，口径为全部去重包名）（学术，UT San Antonio + Virginia Tech + Oklahoma，2024，即 USENIX Security 2025 论文；以上数字 2026-08-15 对照论文原文复核）
...
- [PyPI JSON API](https://docs.pypi.org/api/json/) / [npm registry API](https://github.com/npm/registry/blob/main/docs/REGISTRY-API.md) —— 支撑②的全部命令与示例输出（官方 + 2026-08-15 本机实测）
- [pip changelog: 25.1](https://pip.pypa.io/en/stable/news/) —— 支撑时效声明里"`pip index versions` 的 experimental 警告已在 25.1（2025-04-26）移除"（官方，2026-08-15 复核）
- [Context7 官方 README](https://github.com/upstash/context7) —— 支撑④的工具名 `resolve-library-id` + `query-docs`（官方，2026-08-15 复核；`get-library-docs` → `query-docs` 改名于 v2.0.0（2025-12-29），见社区 issue [AndyMik90/Aperant#1669](https://github.com/AndyMik90/Aperant/issues/1669)）
...
- [awesome-cursorrules](https://github.com/PatrickJS/awesome-cursorrules) —— 支撑③"引依赖前的三道门槛 + 20 行以下不引依赖"这套规则形态（社区规则集合，2026-08 复核；6 个月 / 30 行 / 20 行为经验阈值，未见官方确证）
...
[^1]: [Pyright configuration](https://github.com/microsoft/pyright/blob/main/docs/configuration.md)（官方文档，2026-08-15 复核）：`reportMissingImports` 的默认值是 `"error"`。
...
[^5]: [arXiv:2406.10279 §5.1 / §5.3](https://arxiv.org/abs/2406.10279)（学术，2024；2026-08-15 对照论文原文复核）：223 万个被推荐的包里 440,445 个（19.7%）是幻觉，含 205,474 个唯一不存在包名；58% 的情况下同一幻觉包会在 10 次迭代里重复出现不止一次。
...
<sub>**时效**：命令与工具行为于 2026-08-15 实测复核 —— Context7 现行工具名为 `resolve-library-id` + `query-docs`（官方 README 当日核对；`get-library-docs` → `query-docs` 的改名发生在 Context7 MCP v2.0.0，2025-12-29，社区 issue 与上游提交可查）；`import/no-unresolved` 能挡幻觉包而 Ruff 没有等效规则；`pip index versions` 的 experimental 警告已在 pip 25.1（2025-04-26）移除（pip changelog 当日核对），但输出走 stderr；pyright `reportMissingImports` 默认 `"error"` 当日对照官方配置文档核对。**已知不确定**：diffray 的"RAG 降幻觉 60-80%"来自厂商博客转引，无独立复现，只作量级参考。**已知不确定（补）**：③ 的三道门槛里，"6 个月内有发布"是社区常用的维护活跃度分界，未见官方确证；"30 行内自实现"与"20 行以下任务不引依赖"是个人经验取值，无外部依据，团队可自行调整；"AI 偏好能引包就引包"为个人与社区观察，未见系统性测量。**易变**：MCP 工具名是本篇最易变的一类信息，报 `tool not found` 时用 `/mcp` 查实际名；registry 返回字段与 pyright 版本号会随时间变；② ③ 的 PyPI / npm 查询命令与示例输出（`langchain-openai` 1.5.1、`requests` 2.34.2 / 2026-05-14、`left-pad` 2024-04-16）于 2026-08-15 实跑验证；① 的 eslint 自测于 2026-08-15 在最小 flat config 项目实跑验证。"幻觉类型 × 查防手段"矩阵不依赖版本，长期有效。</sub>
